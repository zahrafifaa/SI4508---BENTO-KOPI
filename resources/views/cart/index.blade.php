...
{{-- <table id="cart" class="table table-striped table-sm mt-4">
    <thead>
        <tr>
            <th scope="col" width='30px' class="text-center">Product</th>
    ...
    <tbody>
        ...
        <tr>
            <td class="d-flex align-items-center">
                <div>
                    <img src="{{ $cart->menu->gambar }}" alt="Product Image" style="max-width: 100px;">
                </div>
                <div>
                    <!-- Form untuk mengurangi jumlah -->
                    <form action="{{ route('cart.reduceQuantity') }}" method="POST">
                        @csrf
                        <input type="hidden" value="{{ $cart->menu_id }}" name="menu_id">
                        <button class="minus-icon m-0" style="border:none ; background-color: transparent;" >
                            <i data-feather='minus'></i>
                        </button>
                    </form>
                </div>
                ...
                <div>
                    ...
                    <form action="{{ route('cart.increaseQuantity') }}" method="POST">
                        @csrf
                        <input type="hidden" value="{{ $cart->menu_id }}" name="menu_id">
                        <button class="d-flex align-item-center plus-icon" style="border:none ; background-color: transparent;"><i data-feather='plus'></i></button>
                    </form>
                </div>
